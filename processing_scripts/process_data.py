...
class DataWrangler:
    def __init__(self, fout = '../data_processed', cutoff_cases_expression_rnaseq = 2):
        ...
        self.formats_datCategory = { "biospecimen": ["svs", "jpeg 2000"], "clinical": ["bcr xml"], "copy number variation": ["txt","tsv"], "dna methylation": ["txt"], "proteome profiling": ["tsv"], "simple nucleotide variation": ["maf"], "transcriptome profiling": ["tsv"] }
    ...
    def _get_file_by_uuid(self, fsodir, uuid):
        ...
        if( not os.path.isfile(opath) or os.path.getsize(opath) == 0 ):
    ...
    def _get_cases_metadata(self, project):
        ...
        map_case = self._get_map_case_file(odir)
        dt = {}
        for el in d:
            case = map_case[ el['uuid'] ]
            dt[case] = el
    ...
    def _select_cases_with_normalAndTumor(self, odir, data_type = None):
        pmeta = os.path.join(odir, "files_metadata.tsv")
        df = pd.read_csv( pmeta, sep='\t')
