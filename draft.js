...
uuids = dat.hits.map( e => e.file_id )
...
dat.hits.forEach( el => {
	...
	let stage = el.samples[0].tumor_descriptor;
	y[uuid] = { "tissue": tissue, "stage": stage };

	let races = Object.keys( filesdf['race'] );
	let dr = e.cases[0].demographic.race;
	sobj["race"] = dr;
	...
	dr = e.cases[0].demographic.gender;
	sobj["gender"] = dr;
	...
	dr = e.cases[0].demographic.ethnicity;
	sobj["ethnicity"] = dr;
	...
	let diags = el.cases[0].diagnosis;
	for( let dg of diags ){
